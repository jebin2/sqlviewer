...
export const SchemaVisualizer: React.FC<SchemaVisualizerProps> = ({ tables }) => {
  ...
  const HEADER_HEIGHT = 48; // Header padding + content
  const ROW_HEIGHT = 28;    // Each column row height
  const ROW_PADDING = 8;    // Top padding in columns section
  ...
  // Helper to draw bezier curves connecting FK columns to their target columns
    ...
    for (const table of tables) {
      ...
      table.foreignKeys.forEach((fk, fkIdx) => {
        ...
        if (targetTable && positions[targetTable.name]) {
          ...
          // Find the actual column index for the FK column in source table
          const sourceColIndex = table.columns.findIndex(col => col.name === fk.from);
          // Find the actual column index for the target column in destination table
          const targetColIndex = targetTable.columns.findIndex(col => col.name === fk.toColumn);

          // Calculate Y positions based on actual column positions
          // Header: 48px, each row: 28px, row padding: 8px
          const sourceRowY = sourceColIndex >= 0
            ? HEADER_HEIGHT + ROW_PADDING + (sourceColIndex * ROW_HEIGHT) + (ROW_HEIGHT / 2)
            : HEADER_HEIGHT + ROW_PADDING + (fkIdx * ROW_HEIGHT) + (ROW_HEIGHT / 2);

          const targetRowY = targetColIndex >= 0
            ? HEADER_HEIGHT + ROW_PADDING + (targetColIndex * ROW_HEIGHT) + (ROW_HEIGHT / 2)
            : HEADER_HEIGHT + ROW_PADDING + (ROW_HEIGHT / 2);

          // Determine connection side based on relative position
          const sourceOnLeft = startPos.x < endPos.x;

          let startX: number, endX: number;

          if (sourceOnLeft) {
            // Source is on left: connect from right of source to left of target
            startX = startPos.x + CARD_WIDTH;
            endX = endPos.x;
          } else {
            // Source is on right: connect from left of source to right of target
            startX = startPos.x;
            endX = endPos.x + CARD_WIDTH;
          }

          const startY = startPos.y + sourceRowY;
          const endY = endPos.y + targetRowY;

          // Calculate control points for smooth curves
          const horizontalDist = Math.abs(endX - startX);
          const controlOffset = Math.min(80, horizontalDist * 0.4);

          const cp1x = sourceOnLeft ? startX + controlOffset : startX - controlOffset;
          const cp1y = startY;
          const cp2x = sourceOnLeft ? endX - controlOffset : endX + controlOffset;
          const cp2y = endY;

          const path = `M ${startX} ${startY} C ${cp1x} ${cp1y}, ${cp2x} ${cp2y}, ${endX} ${endY}`;

          // Generate a color based on table name + FK column for variety
          const colorHash = (table.name + fk.from).split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
          ...
          const arrowPoints = sourceOnLeft
            ? `${endX},${endY} ${endX - 8},${endY - 4} ${endX - 8},${endY + 4}`
            : `${endX},${endY} ${endX + 8},${endY - 4} ${endX + 8},${endY + 4}`;

          lines.push(
            <g key={`${table.name}-${fk.from}-${fk.toTable}-${fk.toColumn}`}>
              <path
                d={path}
                fill="none"
                stroke={color}
